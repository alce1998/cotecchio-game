...
export default function LoginModal({ open, onClose, onSuccess }: LoginModalProps) {
  ...
  const handleGoogleCustomSubmit = (e: React.FormEvent) => {
    ...
    const formattedName = googleNickname.trim() || (fallbackName.charAt(0).toUpperCase() + fallbackName.slice(1));
  ...
  const handleConfirmGoogleNickname = (e: React.FormEvent) => {
    ...
    loginGoogle.mutate({ credential: pendingCredential, nickname: googleNickname.trim() || undefined });
  };
      ...
      <section className="setup-card" style={{ maxWidth: 460, padding: 32, position: "relative" }}>
        ...
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <p style={{ fontSize: 13, color: "#6e583d", margin: 0 }}>
            Scegli il tuo nickname al tavolo per registrare i tuoi risultati e scalare la classifica stagionale
          </p>
        </div>
          ...
          <form onSubmit={handleConfirmGoogleNickname} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ padding: 12, borderRadius: 8, background: "#f0f7ff", border: "1px solid #4285F4" }}>
              <p style={{ margin: 0, fontSize: 13, color: "#1a73e8", fontWeight: 600 }}>
                ✓ Account Google Autenticato
              </p>
              <p style={{ margin: "4px 0 0", fontSize: 12, color: "#444" }}>
                Scegli il Nickname con cui apparirai al tavolo e in classifica:
              </p>
            </div>

            <label style={{ display: "flex", flexDirection: "column", gap: 6, textAlign: "left" }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: "#4a3928" }}>
                Nickname per le partite online:
              </span>
              <input
                type="text"
            ...
            >
              {loginGoogle.isPending ? "Salvataggio..." : "Entra al Tavolo con Google"} <LogIn size={18} />
            </button>
          </form>
        ) : (
          <>
                ...
                <div style={{ width: "100%" }}>
                  {!showGooglePrompt ? (
                    ...
                    <form onSubmit={handleGoogleCustomSubmit} style={{ display: "flex", flexDirection: "column", gap: 10, width: "100%" }}>
                      ...
                      />
                      <label style={{ fontSize: 12, fontWeight: 600, color: "#4a3928", textAlign: "left" }}>
                        Scegli il tuo Nickname al tavolo:
                      </label>
                      <input
                        type="text"
                        value={googleNickname}
                        onChange={(e) => setGoogleNickname(e.target.value)}
                        placeholder="Es. Marco, IlPiacentino..."
                        required
                        maxLength={30}
                        style={{
                          width: "100%",
                      ...
                      <button
                        ...
                        disabled={loginQuick.isPending || !googleEmail.trim() || !googleNickname.trim()}
                      >
