...
import { Bell, BookOpen, Camera, ChevronRight, CircleHelp, Copy, KeyRound, LogIn, Medal, MessageCircle, Mic, MicOff, Pause, RotateCcw, Send, Timer, Trophy, User, Users, Video, VideoOff, Wifi, X } from "lucide-react";
...
export default function Home() {
  ...
  const [customNickname, setCustomNickname] = useState(user?.name ?? "");
  const [pauseMinutesOpen, setPauseMinutesOpen] = useState(false);
  const [selectedPauseMinutes, setSelectedPauseMinutes] = useState(2);
  const [spectateChoiceOpen, setSpectateChoiceOpen] = useState(false);
  ...
  const joinPrivate = trpc.match.joinPrivate.useMutation({ onSuccess: (data) => { setOnlineRoomId(data.room.id); setSetupOpen(false); setSpectateChoiceOpen(false); }, onError: reportError });
  ...
  const requestPauseProposal = trpc.match.requestPauseProposal.useMutation({ onSuccess: () => { setPauseMinutesOpen(false); onlineSnapshot.refetch(); toast.success("Richiesta di pausa inviata al tavolo."); }, onError: reportError });
  const votePauseProposal = trpc.match.votePauseProposal.useMutation({ onSuccess: () => onlineSnapshot.refetch(), onError: reportError });
  const setPauseReady = trpc.match.setPauseReady.useMutation({ onSuccess: () => onlineSnapshot.refetch(), onError: reportError });
  ...
  const saveNickname = trpc.profile.setNickname.useMutation({ onSuccess: () => { utils.auth.me.invalidate(); onlineSnapshot.refetch(); toast.success("Nickname aggiornato."); }, onError: reportError });
  ...
  function startOnlineMatch() {
    ...
    if (customNickname.trim() && customNickname !== user?.name) {
      saveNickname.mutate({ nickname: customNickname });
    }
    if (onlineEntry === "private-create") createPrivate.mutate({ scoreLimit: 100 });
    else if (onlineEntry === "private-join") setSpectateChoiceOpen(true);
    else joinOnline.mutate({ scoreLimit: 100 });
  }
  ...
  function pauseTurn() {
    if (onlineRoomId) {
      if (onlinePaused) {
        setPauseReady.mutate({ roomId: onlineRoomId });
        return;
      }
      setPauseMinutesOpen(true);
      return;
    }
    if (pauseUsed || game.turn !== 0 || game.phase !== "playing") return;
    setPauseUsed(true);
    setPauseSeconds(60);
    setPaused(true);
  }
  ...
  return <main className={`game-shell table-theme-${tableTheme}`} style={{ "--table-image": `url(${activeTableTheme.image})`, "--felt-image": `url(${FELT_IMAGE})` } as React.CSSProperties}>
    ...
    <header className="game-header"><button className="brand" onClick={() => setSetupOpen(true)} aria-label="Configurazione partita"><img src={LOGO_IMAGE} alt="" /><span><strong>Cotecchio</strong><small>Traversone</small></span></button><div className="header-status"><span className="live-dot" />{isOnline ? "Tavolo online" : "Partita locale"} <span>·</span> mano {game.roundIndex}</div><div className="header-actions">{isAuthenticated ? <button className="user-stamp" onClick={() => setProfileOpen(true)}>{user?.name ?? "Giocatore"}</button> : <button className="user-stamp" onClick={() => setLoginModalOpen(true)}>Accedi</button>}<button className="icon-button" onClick={() => setRulesOpen(true)} aria-label="Regole"><CircleHelp size={19} /></button><button className="icon-button" onClick={() => setLeaderboardOpen(true)} aria-label="Classifica stagionale"><Medal size={19} /></button><button className="score-button" onClick={() => setScoreOpen(true)}><Trophy size={16} /> Match</button></div></header>
    <section className="playfield" aria-label="Tavolo di gioco"><div className="table-ring"><div className="felt-surface" /></div>{visibleTableEffect && <div className={`table-omen ${visibleTableEffect} ${previewEffect ? "preview" : ""}`} role="status"><span>{visibleTableEffect === "pelliccione" ? "✦" : visibleTableEffect === "cappotto" ? "✹" : "♛"}</span><strong>{visibleTableEffect === "pelliccione" ? "Pelliccione!" : visibleTableEffect === "cappotto" ? "Cappotto!" : "Partita conclusa"}</strong><small>{visibleTableEffect === "pelliccione" ? "Asso di bastoni · 6 punti" : visibleTableEffect === "cappotto" ? "Sedici punti al tavolo" : "I conti del tavolo sono definitivi"}</small></div>}{visibleTrickToast && <div className={`trick-score dynamic ${demoScenario === "presa" ? "preview" : ""}`} style={tablePosition(visibleTrickToast.playerId, game.playerCount, 35, 27)}><strong>+{visibleTrickToast.points % 1 === 0 ? visibleTrickToast.points : visibleTrickToast.points.toFixed("2").replace(".", ",")}</strong><span>presa</span></div>}{game.players.slice(1).map((player) => { const participant = orderedOnlinePlayers[player.id]; const media = participant ? visibleMediaMembers.find((member) => member.userId === participant.userId) : communicationDemo ? visibleMediaMembers[player.id] : undefined; return <article className="opponent-seat dynamic" style={tablePosition(player.id, game.playerCount, 43, 34)} key={player.id}>{(isOnline || communicationDemo) && <PlayerMediaTile name={player.name} avatarUrl={participant?.avatarUrl ?? null} stream={media ? roomMedia.remoteStreams[media.userId] : undefined} videoEnabled={!mobileAudioOnly && (media?.videoEnabled ?? false)} audioEnabled={media?.audioEnabled ?? false} />}<div className="opponent-name"><span className={game.turn === player.id ? "turn-lamp active" : "turn-lamp"} />{player.name}<small>{player.score} pt</small></div><MiniFan count={player.hand.length} /></article>; })}
      <section className="trick-zone"><div className="lead-indicator"><SuitPip suit={game.leadSuit} /><span>{leadLabel}</span></div><div className={`trick-cards ${game.phase === "resolving" ? "resolving" : ""}`}>{game.trick.map(({ playerId, card }) => <div className="trick-card dynamic" style={tablePosition(playerId, game.playerCount, 34, 26)} key={`${playerId}-${card.id}`}><PiacentineCard card={card} small /><span>{game.players[playerId].name}</span></div>)}{!game.trick.length && <div className="center-emblem"><img src={LOGO_IMAGE} alt="Logo Cotecchio" style={{ width: 56, height: 56, borderRadius: "50%", objectFit: "cover", border: "2px solid rgba(241,205,133,0.5)", boxShadow: "0 4px 12px rgba(0,0,0,0.4)" }} /><span>{game.turn === 0 ? "Tocca a te" : `${activePlayer?.name ?? ""} apre`}</span></div>}</div></section>
    ...
    {(paused || onlinePaused) && <div className="pause-scrim"><section><Pause size={30} /><p className="eyebrow">Tavolo in attesa</p><h2>{isOnline ? "Pausa concordata al tavolo" : "Pausa in corso"}</h2><strong>00:{String(onlinePaused ? onlinePauseSeconds : pauseSeconds).padStart(2, "0")}</strong>{isOnline && <p style={{ fontSize: "0.85rem", opacity: 0.85, margin: "6px 0 12px 0" }}>Pronti al rientro: {onlineSnapshot.data?.pauseReadyUserIds?.length ?? 0} / {onlineSnapshot.data?.players?.length ?? 0}</p>}<button onClick={() => onlinePaused && onlineRoomId ? setPauseReady.mutate({ roomId: onlineRoomId }) : setPaused(false)}>Sono pronto a giocare</button></section></div>}
    ...
    {isOnline && onlineSnapshot.data?.pauseProposal && <Overlay title="Richiesta di Pausa" onClose={() => undefined}><p className="overlay-intro"><strong>{onlineSnapshot.data.pauseProposal.requesterName}</strong> ha richiesto una pausa di <strong>{onlineSnapshot.data.pauseProposal.minutes} minuti</strong> al tavolo.</p><p className="overlay-intro" style={{ fontSize: "0.9rem", opacity: 0.8 }}>Approvata da {onlineSnapshot.data.pauseProposal.acceptedUserIds.length} su {onlineSnapshot.data.pauseProposal.totalRequired} giocatori.</p>{!onlineSnapshot.data.pauseProposal.hasVoted ? <div className="departure-actions"><button className="primary-action" disabled={votePauseProposal.isPending} onClick={() => votePauseProposal.mutate({ roomId: onlineRoomId!, vote: "accept" })}>Accetta Pausa</button><button className="quiet-action" disabled={votePauseProposal.isPending} onClick={() => votePauseProposal.mutate({ roomId: onlineRoomId!, vote: "reject" })}>Rifiuta</button></div> : <p className="overlay-intro">Hai votato. Attendo la risposta degli altri giocatori del tavolo…</p>}</Overlay>}
    {pauseMinutesOpen && <Overlay title="Richiedi Pausa" onClose={() => setPauseMinutesOpen(false)}><p className="overlay-intro">Seleziona la durata della pausa da sottoporre all’approvazione del tavolo:</p><div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 10, margin: "16px 0" }}>{[1, 2, 3, 5, 10].map((min) => <button type="button" key={min} className={selectedPauseMinutes === min ? "primary-action" : "quiet-action"} style={{ padding: "12px 6px", fontSize: "1rem", fontWeight: "bold" }} onClick={() => setSelectedPauseMinutes(min)}>{min} min</button>)}</div><button className="primary-action" style={{ marginTop: 12 }} disabled={requestPauseProposal.isPending} onClick={() => { if (onlineRoomId) requestPauseProposal.mutate({ roomId: onlineRoomId, minutes: selectedPauseMinutes }); }}>Invia richiesta al tavolo</button></Overlay>}
    {spectateChoiceOpen && <Overlay title="Partita già avviata" onClose={() => setSpectateChoiceOpen(false)}><p className="overlay-intro">Questa sala privata ha una partita già in corso. Come desideri entrare?</p><div className="departure-actions" style={{ flexDirection: "column", gap: 10 }}><button className="primary-action" disabled={joinPrivate.isPending} onClick={() => joinPrivate.mutate({ inviteCode, mode: "player" })}>Entra al tavolo come Giocatore (punteggio aggiornato)</button><button className="quiet-action" style={{ padding: 12 }} disabled={joinPrivate.isPending} onClick={() => joinPrivate.mutate({ inviteCode, mode: "spectate" })}>Osserva la partita (Spettatore)</button></div></Overlay>}
